  ...
  return (
    <header>
      <div className="relative bg-white">
        ...
        {isMobileMenu && <MobileMenu onClose ={setIsMobileMenu}/> }
      </div>
    </header>
  );
};
...
const SignButtons = () => {
  ...
  return (<>
    {user ? <span className="text-lg">{user.username}</span>  :    <div className="hidden md:flex items-center justify-end md:flex-1 lg:w-0">
      <Link
        to={"/login"}
...
const NavLinks = () => {
  return (
    <nav className="hidden md:flex space-x-10 justify-end" >
      <Link
        to="/"
